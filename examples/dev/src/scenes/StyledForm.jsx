...
import {
    FormProvider, TextInput, CheckboxInputBlock, RadioButtonBlock, SelectBlock, RadioButtonBlockEx, TextareaBlock,
} from 'examples/styled.components'

import { isEmailValid } from 'examples/validators'
import 'examples/reset.css'
...
const validatorExample = (value, errors, formState, invalidator) => {
    ...
    setTimeout(
        () => {
          if(isEmailAcceptable(value)){ return }
          invalidator(value, [ 'yahoo emails are not acceptable' ])
        },
        2000,
    )
    return isEmailValid(value) ? [] : [ 'please use valid email' ]// ['Sync A', 'Sync B']
